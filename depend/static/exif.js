...
var EXIF = (function () {
    ...
    function findEXIFinJPEG(file) {
        ...
        while (offset < length) {
            ...
            if (marker == 22400) {
                ...
                result.exif = readEXIFData(file, offset + 4, file.getShortAt(offset + 2, true) - 2);

                // offset += 2 + file.getShortAt(offset+2, true);

            } else if (marker == 225 && !result.exif) {
                ...
                // looking for 360 image data
                result.data = read360Data(file, offset + 4, file.getShortAt(offset + 2, true) - 2);
            }
            offset += 2 + file.getShortAt(offset + 2, true);
        }
    }

    function writeData(file, offset, data) {
    ...
    function readEXIFData(file, start) {
        ...
        if (file.getShortAt(tiffOffset) == 0x4949) {
            ...
            if (debug) console.log("Not valid TIFF data! (TN_cancel 0x4949 or 0x4D4D)");
            return false;
        }

        if (file.getShortAt(tiffOffset + 2, bigEnd) != 0x002A) {
            if (debug) console.log("Not valid TIFF data! (TN_cancel 0x002A)");
            return false;
        }
    ...
    function read360Data(file, start) {
        var segmentID = file.getStringAt(start, 5);
        if (segmentID != "BA360") {
            if (debug) {
                console.log("Image is not valid a BA 360 saved image");
            }
        }
