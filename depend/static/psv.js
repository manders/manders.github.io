...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    define(['../../ext/three', 'uevent', 'dot/doT'], factory);
  }
  else if (typeof module === 'object' && module.exports) {
    module.exports = factory(require('../../ext/three'), require('../../ext/uevent'), require('dot/doT'));
  }
  else {
...
function PhotoSphereViewer(options) {
  ...
  // Exception: ba-panoramic-images - allowing full pan correction
  this.config.sphere_correction.pan = PSVUtils.parseAngle(this.config.sphere_correction.pan, false);
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                if (xhr.status === 200 || xhr.status === 201 || xhr.status === 202 || xhr.status === 0) {
                    ...
                    // var stuff = this.readInfoFromData(responseArray);

                    var a         = responseText.indexOf('<x:xmpmeta'), b = responseText.indexOf('</x:xmpmeta>');
                    var data      = responseText.substring(a, b);
                    var pano_data = null;
...
PhotoSphereViewer.prototype.readInfoFromData = function (data) {
    var offset = 0;
    if (this.getByteAt(data, 0) == 0xFF && this.getByteAt(data, 1) == 0xD8) {
        this.tags = this.readJPEGInfo(data);
        return this.tags;
    }
    return null;
};

PhotoSphereViewer.prototype.getByteAt = function (data, offset) {
    return data.charCodeAt(offset) & 0xFF;
};

PhotoSphereViewer.prototype.getShortAt = function (data, offset, bigEndian) {
    var short = bigEndian ?
        (this.getByteAt(data, offset) << 8) + this.getByteAt(data, offset + 1)
        : (this.getByteAt(data, offset + 1) << 8) + this.getByteAt(data, offset)
    if (short < 0) short += 65536;
    return short;
};

PhotoSphereViewer.prototype.readJPEGInfo = function (data) {
    var SIZE_FIRST = 0xc0;
    var SIZE_LAST  = 0xc3;
    var x, y, comps;
    var id         = 'could not determine JPEG size';
    var size       = data.getLength();

    var ptr = 1;
    this.getByteAt(data, ptr);

    while (ptr < size) {
        var marker = this.getByteAt(data, ptr + 1);
        var code   = this.getByteAt(data, ptr + 2);
        var len    = this.getShortAt(data, ptr + 3, true);
        ptr += 4;
        if (marker != 0xff) {
            throw 'JPEG marker not found';
        } else if (code >= SIZE_FIRST && code <= SIZE_LAST) {
            len = 5;
            this.getByteAt(data, ptr + 1);
            y     = this.getShortAt(data, ptr + 2, true);
            x     = this.getShortAt(data, ptr + 4, true);
            comps = this.getByteAt(data, ptr + 6);
            id    = 'JPEG';
            break;
        } else {
            ptr += len - 2;
...
PhotoSphereViewer.prototype.getAllFields = function () {
    return this.tags;
};

PhotoSphereViewer.prototype.getField = function (name) {
                ...
                var info = new ImageInfo(
                    ...
                    (function (element){
                        return function (imageinfo){
                            ...
                            //     return shelf;
                            // }
                            // $j(element).after('<pre>'+ dumper([], imageinfo.getAllFields(), '').join('\n') +'</pre>');
                            if (imageinfo) {

                                var item, exif = imageinfo.getAllFields();
                                if (exif.length>0) {
                                    // generate the exif page
...
PhotoSphereViewer.prototype._createScene = function () {
    ...
    this.renderer = PhotoSphereViewer.SYSTEM.isWebGLSupported && this.config.webgl ? new THREE.WebGLRenderer(opt) : new THREE.CanvasRenderer();
...
<div class="psv-markers-list-container"> \
  ...
  exifList: '\
<div class="psv-markers-list-container"> \
  <h1 class="psv-markers-list-title">{{= it.config.lang.exif }}</h1> \
  <ul class="psv-markers-list"> \
  {{~ it.markers: marker }} \
    <li data-psv-marker="{{= marker.id }}" class="psv-markers-list-item {{? marker.className }}{{= marker.className }}{{?}}"> \
      {{? marker.image }}<img class="psv-markers-list-image" src="{{= marker.image }}"/>{{?}} \
  ...
  facts: '\
<div class="psv-markers-list-container"> \
  <h1 class="psv-markers-list-title">{{= it.config.lang.facts }}</h1> \
  <ul id="psv-facts-list"> \
  </ul> \
</div>'
};
...
PhotoSphereViewer.prototype._compassPts = [
    'N',
    'NNE',
    'NE',
    'ENE',
...
PhotoSphereViewer.prototype._displayCompassReading = function (lon) {
    var degrees = Math.floor(Math.degrees(lon));
    $j('#_ba_pano_compassDegrees').html(degrees);

    var label = '';
    var idx = Math.floor((degrees % 360)/ 22.5);
    $j('#_ba_pano_compassNS').html(this._compassPts[idx]);
};
...
PhotoSphereViewer.prototype._move = function (evt, log) {
    if (this.prop.moving) {
        ...
        if ($j("#_ba_pano_360confirm").is(':visible')){
            this._displayCompassReading(0);
        }
        else {
            this._displayCompassReading(this.prop.position.longitude);
            // GE heading notifications
            var latBounds = 24;//degrees
            var lat = Math.degrees(this.prop.position.latitude);
            var latWithinBounds = lat >= (-latBounds) && (lat <= latBounds);
            if (typeof window._ba_pano_updateHeading != 'undefined' && latWithinBounds){
              window._ba_pano_updateHeading(Math.degrees(this.prop.position.longitude));
            }
        }
 ...
 * @description The return value is a Promise because the is TN_cancel guaranty the animation can be stopped synchronously.
...
/**
 * @summary Toggles the visibility of exif list
 */
PSVHUD.prototype.toggleExifList = function() {
  this.prop.markersButton = null;
...
/**
 * @summary Toggles the visibility of facts list
 */
PSVHUD.prototype.toggleFactsSidebar = function() {
  this.prop.markersButton = null;
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
/* @summary Opens side panel with exif information
* @fires module:components.PSVHUD.filter:render-exif-list
*/
PSVHUD.prototype.showExifList = function() {
   ...
   * @returns {PSVMarker[]}
   */
  var html = this.psv.config.templates.exifList({
    markers: this.psv.change('render-exif-list', info),
    config: this.psv.config
  });
...
/**
 * @summary Closes side panel if it contains the exif list
 */
PSVHUD.prototype.hideExifList = function() {
  if (this.prop.panelOpened) {
    this.psv.panel.hidePanel();
  }
};
...
//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ...
  * @fires module:components.PSVHUD.filter:render-exif-list
  */
  PSVHUD.prototype.showFactsList = function() {
  ...
  /**
   * @summary Closes side panel if it contains the Facts list
   */
  PSVHUD.prototype.hideFactsList = function() {
    if (this.prop.panelOpened) {
      this.psv.panel.hidePanel();
    }
  };
  ...
  this.config.forEach(function(button) {
    if (typeof button === 'object') {
      ...
      switch (button) {
        ...
        case PSVNavBarExifButton.id:
          this.items.push(new PSVNavBarExifButton(this));
          break;

        case PSVNavBarFactsButton.id:
          this.items.push(new PSVNavBarFactsButton(this));
          break;
...
PSVPanel.prototype.showPanel = function(content, noMargin) {
  ...
  PSVUtils.toggleClass(this.content, 'psv-panel-content--TN_cancel-margin', noMargin === true);
...
PSVPanel.prototype._onMouseUp = function(evt) {
  if (this.prop.mousedown) {
    ...
    this.content.classList.remove('psv-panel-content--TN_cancel-interaction');
  }
};
...
PSVPanel.prototype._startResize = function(evt) {
  ...
  this.content.classList.add('psv-panel-content--TN_cancel-interaction');
};
 ...
 * @memberof module:components/buttons
 */
function PSVNavBarExifButton(navbar) {
  PSVNavBarButton.call(this, navbar);

  this.create();
}

PSVNavBarExifButton.prototype = Object.create(PSVNavBarButton.prototype);
PSVNavBarExifButton.prototype.constructor = PSVNavBarExifButton;

PSVNavBarExifButton.id = 'exif';
PSVNavBarExifButton.className = 'psv-button psv-button--hover-scale psv-exif-button';
PSVNavBarExifButton.icon = null;

/**
 * @override
 * @description Toggles markers list
 */
PSVNavBarExifButton.prototype._onClick = function() {
  this.psv.hud.toggleExifList();
};

// Facts button --------------------------
  function PSVNavBarFactsButton(navbar) {
    PSVNavBarButton.call(this, navbar);

    this.create();
  }

  PSVNavBarFactsButton.prototype = Object.create(PSVNavBarButton.prototype);
  PSVNavBarFactsButton.prototype.constructor = PSVNavBarFactsButton;

  PSVNavBarFactsButton.id = 'facts';
  PSVNavBarFactsButton.className = 'psv-button psv-button--hover-scale psv-facts-button';
  PSVNavBarFactsButton.icon = null;

  /**
   * @override
   * @description Toggles markers list
   */
  PSVNavBarFactsButton.prototype._onClick = function() {
    // this.psv.hud.toggleFactsSidebar();
    if (typeof _ba_pano_onToggleFacts != 'undefined'){
      _ba_pano_onToggleFacts();
    }
  };
